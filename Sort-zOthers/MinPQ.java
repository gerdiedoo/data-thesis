 ...
 * This priority queue implements an insert() method and 
 * a delMin() method that returns the minimum value of the 
 * heap. It is implemented from MaxPQ by simply inverting 
 * the result of the keys comparison in the less() method.
 *
 * @param <Key>
 */
package sorting;
...
public class MinPQ<Key extends Comparable<Key>> {
  ...
  /**
   * We take the smallest key off the top, put the item from
   ...
   */
  public Key delMin() {
    Key max = pq[1]; // retrieve min key from top.
  ...
  public static void main(String[] args) {
    ...
    while (!StdIn.isEmpty() && numOperations < maxOperations) {
      ...
      else if (!pq.isEmpty()) StdOut.print(pq.delMin() + " "); // remove min
    }
